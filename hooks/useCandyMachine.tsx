...
export default function useCandyMachine() {
  const apiUrl = process.env.NEXT_PUBLIC_WHITELIST_API_URL
  ...
  const [mintPresaleDate, setMintPresaleDate] = useState(
    new Date(parseInt(process.env.NEXT_PUBLIC_CANDY_PRESALE_DATE!, 10))
  );
  ...
  useEffect(() => {
    (async () => {
      ...
      const data = await fetch(`${apiUrl}/whitelisted/member/${wallet.publicKey.toString()}`)
  ...
  const startMint = async (isMintLive: boolean) => {
    try {
      if (!isMintLive) {
        let res = await fetch(`${apiUrl}/whitelisted/member/${wallet?.publicKey?.toString()}`, { method: "GET" })
      ...
      if (wallet.connected && candyMachine?.program && wallet.publicKey) {
        ...
        if (!status?.err) {
          ...
          if (!isMintLive) {
            await fetch(`${apiUrl}/whitelisted/update/${wallet.publicKey.toString()}/${process.env.NEXT_PUBLIC_SECRET_KEY}`, {
              method: "PUT",
              headers: {
